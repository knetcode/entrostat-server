...
- Valid verification (returns 200 with `{success: true, valid: true}`)
- Invalid OTP codes (returns 400 with `{success: false}`)
- Expired OTPs (30 second expiry, returns 400 with `{success: false}`)
- Already used OTPs (returns 400 with `{success: false}`)
- No active OTP found (returns 400 with `{success: false}`)
- Invalid formats (returns 400 with validation errors)
...
- **Verify Response Format**:
  - Successful verification: HTTP 200 with `{success: true, valid: true}`
  - Failed verification (wrong/expired/used OTP): HTTP 400 with `{success: false, message: "Invalid or expired OTP"}`
  - Validation errors: HTTP 400 with `{success: false, errors: [...]}`
